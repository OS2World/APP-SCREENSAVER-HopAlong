...
{
   static POINTL	 point;
   static double   a,b,c,i,j,oldj;
   static int      k,range,centerx,centery;
   ...
   while(!stop_draw_thread)
      {
      srand ( rand () );
      a = (rand() % (range * 100)) * ((rand()%2)?-1.0:1.0) / 100.0;
      srand ( rand () );      
      b = (rand() % (range * 100)) * ((rand()%2)?-1.0:1.0) / 100.0;
      srand ( rand () );
      c = (rand() % (range * 100)) * ((rand()%2)?-1.0:1.0) / 100.0;
